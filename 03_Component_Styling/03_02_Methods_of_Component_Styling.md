...
**Пример 12.** _Использование медиа-запросов_
...
**Пример 13.** _Комбинирование классов_
...
**Пример 14.** _Использование пользовательских переменных_
...
**Пример 15.** _Использование квадратных скобок_
...
**Пример 16.** _Использование слоя components_
...
**Пример 17.** _Использование слоя utilities_
...
**Пример 18.** _Использование состояний_
...
## Использование pre-styled библиотек
...
**Пример 20.** _Создание карточки с изображением и текстом_
...
**Пример 21.** _Использование Tailwind CSS с Radix UI_
...
| **Метод стилизации**              | **Ключевые особенности**                                                            | **Преимущества**                                                                                         | **Недостатки**                                                                                     | **Когда использовать**                                                                                                 |
| --------------------------------- | ----------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| **Inline styles**                 | Стили задаются в атрибуте `style` через объект JavaScript.                          | - Простота применения.<br>- Удобны для динамических стилей.<br>- Исключают конфликты стилей.             | - Нет поддержки псевдоклассов и медиа-запросов.<br>- Код становится громоздким при сложных стилях. | - Быстрое тестирование/прототипирование.                                                                               |
| **CSS файлы**                     | Использование внешних файлов `.css`, подключаемых через `import`.                   | - Легкость понимания.<br>- Полная поддержка CSS.<br>- Подходит для глобальных и базовых стилей.          | - Возможны конфликты глобальных стилей.<br>- Нет изоляции стилей для компонентов.                  | - Маленькие проекты.<br>- Простые приложения без сложной структуры компонентов.                                        |
| **SCSS**                          | CSS с дополнительным функционалом: переменные, вложенность, миксины.                | - Упрощает создание и поддержку стилей.<br>- Переменные и миксины делают код компактным.                 | - Требуется компиляция.<br>- Увеличивается количество файлов.                                      | - Если важны вложенность и переиспользуемость стилей.                                                                  |
| **CSS модули**                    | Изолированные стили с уникальными именами классов.                                  | - Полная изоляция стилей.<br>- Удобство модульного подхода.                                              | - Увеличение количества файлов.<br>- Требуется импорт для каждого файла стилей.                    | - Когда требуется изоляция стилей для компонентов.                                                                     |
| **CSS-in-JS (styled-components)** | Стили пишутся в JavaScript с помощью библиотеки (например, `styled-components`).    | - Стили изолированы.<br>- Возможность изменения стилей через пропсы.<br>- Поддержка динамических стилей. | - Увеличение размера JS-файлов.<br>- Зависимость от сторонних библиотек.                           | - Если требуется динамическая стилизация.<br>- Подходит для приложений с большим количеством взаимодействий и условий. |
| **Tailwind CSS**                  | Утилитарный CSS-фреймворк с готовыми классами для стилизации.                       | - Быстрая и гибкая стилизация.<br>- Мощные утилитарные классы.<br>- Минимальный объем кастомного CSS.    | - Требует изучения.<br>- Может увеличить размер итогового CSS.                                     | - Когда нужен атомарный подход и готовая типографика.                                                                  |
| **CSS модули + Tailwind CSS**     | Совмещение уникальных классов CSS модулей и мощных утилитарных стилей Tailwind CSS. | - Полная изоляция стилей.<br>- Сочетание преимуществ Tailwind и CSS модулей.                             | - Настройка может быть сложной.<br>- Может привести к смешиванию подходов в коде.                  | - Когда необходима изоляция стилей с преимуществами Tailwind CSS.                                                      |
| **Pre-styled библиотеки**         | Готовые стилизованные компоненты (например, Radix UI, Chakra UI).                   | - Снижение времени разработки.<br>- Легкость интеграции.<br>- Поддержка кастомизации.                    | - Ограничения API.<br>- Возможно, стили не покрывают всех потребностей.                            | - Для ускорения разработки.<br>- Если нужен консистентный UI с минимальной настройкой (например, для админ-панелей).   |

> [!TIP]  
> Выбор метода стилизации зависит от требований проекта, опыта разработчика и предпочтений команды. Важно учитывать особенности каждого метода и выбирать наиболее подходящий для конкретного случая.
